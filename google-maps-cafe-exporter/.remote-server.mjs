...
async function createCandidatesFromScoring(scoringId) {
  ...
  const eligibleRows = (scoring.rows || []).filter((row) => Number(row.score) > 30);
...
function inferCafeNativeLanguage(cafe = {}) {
  ...
  return rules.find(([, matcher]) => matcher.test(context))?.[0] || '';
}
...
function buildClassicLightModel({ cafe, $, url, language, business, assets, typography, colors, documents, useFallbackMenu = false }) {
  ...
  const model = {
    ...
    footer: {
      copyright: classicField(legal.copyright, { sourceUrl, raw: legal.copyright }),
    ...
    templateOptions: classicField({ showMap: Boolean(address.value || cafe.mapsUrl), showAllergens: menu.items.some((item) => item.allergens?.items?.length), showSocials: Boolean(restaurant.socials.value?.length), showBookingButton: Boolean(restaurant.bookingUrl.value), showConvertedPrices: false }, { status: 'derived', note: 'Флаги рекомендованы по доступным данным; пустые секции выключены.' })
  };
...
async function buildClassicLightStandaloneDocumentFromContent(content) {
  ...
  const resolvedContent = { ...content, __classicLightResolved: true };
...
async function auditProductionCafe(cafe, template) {
  if (!cafe.website) {
    ...
    const classicLight = template?.mode === 'classic-light'
      ? buildClassicLightModel({ cafe, $, url: '', language, business: {}, assets, typography, colors: colorRoles, documents: [], useFallbackMenu: true })
      : null;
    return {
      cafe,
  ...
  const classicLight = template?.mode === 'classic-light'
    ? buildClassicLightModel({ cafe: { ...cafe, website: url }, $, url, language, business: extractJsonLdBusinesses($)[0] || {}, assets, typography, colors: colorRoles, documents: discoveredMenus.documents, useFallbackMenu })
    : null;
  if (classicLight) {
...
async function buildProductionAudit({ productionIds = [], templateId = 'classic-light-1' }) {
  ...
  const brands = [];
  for (const cafe of enrichedCafes) brands.push(await auditProductionCafe(cafe, template));
  return {
    id: `prod-${Date.now()}`,
    createdAt: new Date().toISOString(),
    template,
...
async function buildFullProductionAuditViaReference({ productionIds = [], templateId = 'classic-light-1' }) {
  ...
  return buildProductionAudit({ productionIds, templateId });
}
...
app.get('/sites/:slug', (request, response) => {
  if (!pilotMode) return response.status(404).type('text/plain').send('Not found');
  try {
    publishedSiteSlug(request.params.slug);
    response.set('Cache-Control', 'no-store').sendFile(path.join(__dirname, 'public', 'admin.html'));
  } catch {
    response.status(404).type('text/plain').send('Not found');
  }
});
...
app.post('/api/production/audit', async (request, response) => {
  try {
    ...
    const audit = await buildFullProductionAuditViaReference({ productionIds, templateId });
